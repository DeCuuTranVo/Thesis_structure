import torch
# binary_classification  -> convert_y_index -> add_dât_tensor
x_train = torch.load("/mnt/data_lab513/vqtran_data/data_torch_tensor/tensor/x_train_tensor.pt")
x_test = torch.load("/mnt/data_lab513/vqtran_data/data_torch_tensor/tensor/x_test_tensor.pt")
y_train = torch.load("/mnt/data_lab513/vqtran_data/data_torch_tensor/tensor/y_train_tensor.pt")
y_test = torch.load("/mnt/data_lab513/vqtran_data/data_torch_tensor/tensor/y_test_tensor.pt")
...
for i in range(y_train.shape[0]):
    if (y_train[i] == 0) or (y_train[i] == 1) or (y_train[i] == 3):
        x_train_NC_EMCI_AD.append(x_train[i])
        y_train_NC_EMCI_AD.append(y_train[i])
        
for i in range(y_test.shape[0]):
    if (y_test[i] == 0) or (y_test[i] == 1) or (y_test[i] == 3):
        x_test_NC_EMCI_AD.append(x_test[i])
        y_test_NC_EMCI_AD.append(y_test[i])

print(len(x_train_NC_EMCI_AD))
print(len(x_test_NC_EMCI_AD))
print(len(y_train_NC_EMCI_AD))
print(len(y_test_NC_EMCI_AD))

x_train_NC_EMCI_AD = torch.stack(x_train_NC_EMCI_AD)
x_test_NC_EMCI_AD = torch.stack(x_test_NC_EMCI_AD)
y_train_NC_EMCI_AD = torch.stack(y_train_NC_EMCI_AD)
y_test_NC_EMCI_AD = torch.stack(y_test_NC_EMCI_AD)

print(x_train_NC_EMCI_AD.shape)
print(x_test_NC_EMCI_AD.shape)
print(y_train_NC_EMCI_AD.shape)
print(y_test_NC_EMCI_AD.shape)

torch.save(x_train_NC_EMCI_AD, "x_train_NC_EMCI_AD.pt")
torch.save(x_test_NC_EMCI_AD, "x_test_NC_EMCI_AD.pt")
torch.save(y_train_NC_EMCI_AD, "y_train_NC_EMCI_AD.pt")
torch.save(y_test_NC_EMCI_AD, "y_test_NC_EMCI_AD.pt")
##################################AD vs NC dataset#######################################
# x_train_NC_AD = []
# x_test_NC_AD = []
# y_train_NC_AD = []
